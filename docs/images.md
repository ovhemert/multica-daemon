...
To target a specific version of the multica daemon for an agent, include the version as a tag in the image: `ghcr.io/ovhemert/multica-daemon:v<version>-hermes`. The latest release for each variant is also tagged as `latest-<variant>`, for example `ghcr.io/ovhemert/multica-daemon:latest-hermes`.

## CLI Versions

| Multica daemon version | Claude | Codex | Copilot | Gemini | Hermes base image | OpenCode | Pi |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `v0.3.6 (latest)` | `2.1.150` | `0.133.0` | `1.0.54` | `0.43.0` | `main (v0.14.0 at release)` | `1.15.10` | `0.75.5` |
